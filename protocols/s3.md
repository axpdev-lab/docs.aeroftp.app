...
| Secret Key | Your secret access key | Stored in the encrypted vault |
...
| **FileLu S5** | `s5lu.com` | Global, US, EU, AP, ME | 1 GB at signup | FileLu's S3-compatible object storage |
| **Filebase** | `s3.filebase.io` | Global (`auto`) | See [Filebase](/providers/filebase) | Path-style; no object versioning; copies use `CopyObject` |
...
2. Enter your MinIO server URL as the endpoint (e.g. `https://minio.example.lan:9000`, or `http://127.0.0.1:9000` for a server on this machine).
...
::: warning Plain HTTP endpoints
Since v4.2.0 AeroFTP refuses to connect to an `http://` S3 endpoint that is not on this machine unless you accept it for that profile: the request signature authenticates a call but does not encrypt it, so the credentials and every file would cross the network readable. When the endpoint is plain HTTP and not local, the connection form shows a checkbox to accept it for the profile; for a profile edited by hand or imported from another tool, set `allow_cleartext_endpoint` on the profile. `localhost`, `127.0.0.1` and `::1` need no consent; `*.local` names are not exempt, because other machines on the network segment can read the traffic. Using `https://` needs no option.
:::
...
| "The endpoint ... is plain HTTP and is not on this machine" | An `http://` endpoint on another machine without consent | Switch to `https://`, or accept the cleartext endpoint for this profile in the connection form |
...
- For AeroSync with S3, the **size** compare mode remains the safe choice: S3 listings report the upload time as `LastModified`, not the source file's mtime. The CLI `sync` in one direction already accounts for this: an object counts as current when the sizes match and it is not older than the source (2 s tolerance). Objects uploaded by AeroFTP since v4.2.0, or by rclone, also carry the source mtime as `x-amz-meta-mtime`, which `stat` and downloads use.
